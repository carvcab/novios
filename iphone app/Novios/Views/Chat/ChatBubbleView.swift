            ...
            HStack(alignment: .bottom, spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                        ...
                        HStack(spacing: 6) {
                            ...
                            VStack(alignment: .leading, spacing: 1) {
                                Text(message.replyToSenderId == message.senderId ? "Tú" : "Ella/Él")
                                    .font(.system(size: 9, weight: .semibold)).foregroundColor(isFromMe ? .white.opacity(0.7) : ThemeManager.shared.primaryPink)
                                Text(replyText).font(.system(size: 11)).foregroundColor(isFromMe ? .white.opacity(0.6) : .primary.opacity(0.6)).lineLimit(1)
                            }
                        }
                        .padding(.horizontal, 8).padding(.vertical, 4)
                        .background(isFromMe ? Color.white.opacity(0.1) : Color(.systemBackground).opacity(0.4))
                    ...
                    if message.type == .voice {
                        ...
                        Text(message.text ?? "")
                            .font(.system(size: 15, weight: message.type == .kiss || message.type == .hug || message.type == .touch ? .semibold : .regular))
                ...
                .background {
                    if isFromMe {
                        ThemeManager.shared.neonGlowGradient.opacity(0.92)
                    } else {
                        .ultraThinMaterial
                    }
                }
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .stroke(isFromMe ? Color.white.opacity(0.12) : Color.white.opacity(0.25), lineWidth: 0.6)
                )
                .shadow(color: isFromMe ? ThemeManager.shared.primaryPink.opacity(0.12) : .black.opacity(0.04), radius: 8, y: 4)
    ...
    private var voiceContent: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                if isPlaying { Text("Reproduciendo...").font(.system(size: 10)).foregroundColor(isFromMe ? .white.opacity(0.6) : .primary.opacity(0.4)) }
            }
        }
        .padding(2)
    }
